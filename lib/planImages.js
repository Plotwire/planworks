...
// Signed-URL lifetime. A fresh URL is minted every time a project is opened,
// and the print preview and PDF export renew one that has expired
// (SheetParts.jsx), so a drawing left open longer than this still previews
// and exports its plan.
...
// Best-effort delete of stored objects. Only for files no drawing refers to any
// more: a Save As copy shares its source's files, and a replaced plan's file is
// still what the saved row points at until the next save. Check with
// unreferencedPlanPaths (lib/db.js) first. Never throws — an orphaned object is
// harmless; a deleted plan is gone for good.

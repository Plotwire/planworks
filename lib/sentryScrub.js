 ...
 *   - email confirmation links                     /auth/confirm?token_hash=...
...
function scrubCopy(value, depth = 0) {
  ...
  if (value instanceof Error) {
    ...
    // Their own fields (Stripe's requestId and code, Postgres codes, ...) and
    // cause are kept, scrubbed like everything else, for diagnosis.
    const copy = new Error(scrubText(value.message));
    copy.name = value.name;
    copy.stack = scrubText(value.stack);
    for (const key of Object.keys(value)) {
      copy[key] = SECRET_KEY_RE.test(key) ? FILTERED : scrubCopy(value[key], depth + 1);
    }
    if (value.cause !== undefined) copy.cause = scrubCopy(value.cause, depth + 1);
    return copy;
  }

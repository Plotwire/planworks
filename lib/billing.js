...
export async function upsertSubscription(sub, { userId, customerId } = {}) {
  ...
  // Until supabase/subscriptions-cancel-at.sql is applied there is no cancel_at
  // column. Save the row without it rather than fail the webhook -- the app
  // then falls back to current_period_end for the date.
  if (error?.code === "PGRST204" && /cancel_at\b/.test(error.message || "")) {
    console.warn("[billing] subscriptions.cancel_at column missing; saved without it");
    const { cancel_at: _omit, ...rest } = row;
    ({ error } = await admin.from("subscriptions").upsert(rest, { onConflict: "user_id" }));
  }
